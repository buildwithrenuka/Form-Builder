...
FormVerse rebuilds the form-builder experience as a themed product journey. Creators can enter one of three distinct worlds, build forms with structured controls, and publish them with guardrails such as visibility settings, passwords, expiry dates, and response limits. Results are then managed through analytics, CSV export, and response dashboards. Respondents get clean public flows with no-login submission support, while the backend remains fully typed, documented, and production-deployed.
...
- Public and unlisted sharing with custom slugs, passwords, expiry dates, response limits, and optional response editing.
...
4. Respondents open a live link, fill out the form, and submit without needing an account.
...
- Fixing production issues during release, including API base resolution, CORS verification, docs favicon caching, Cloudflare auth and deploy friction, and registration rate-limit edge cases.
...
- Creator and respondent email notifications via Resend.
...
- Password protection, expiry dates, response limits, and optional response editing.
...
| `auth.login`           | Log in and receive a session token |
| `auth.me`              | Fetch the current user |
| `forms.create`         | Create a draft form |
| `forms.update`         | Update metadata, schema, visibility, slug, limits, password, or archive state |
...
| `forms.getBySlug`      | Fetch a public or shared form |
...
- Response notification emails are best-effort and require a valid `RESEND_API_KEY`.
