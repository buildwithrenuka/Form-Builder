...
export function getFormPages(fields: FormField[], values: Record<string, unknown>): FormPage[] {
  ...
  for (const field of activeFields) {
    if (field.type === 'page_break') {
      if (currentPage.fields.length > 0 || pages.length === 0) {
...
export function countInteractiveFields(fields: FormField[], values: Record<string, unknown>): number {
  return getActiveFields(fields, values).filter(isInteractiveField).length;
}